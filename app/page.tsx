...
const terminalLines = [
  { cmd: "npm run dev --users --app", word: "INTELIGÊNCIA" },
  { cmd: "git commit -m feat-pya-v1.0", word: "PRECISÃO" },
  { cmd: "npx create-next-app latest", word: "EXECUÇÃO" },
  { cmd: "pya db push --linked main", word: "INOVAÇÃO" },
  { cmd: "pya host --forward master", word: "EXCELÊNCIA" },
...
function TerminalSection() {
  ...
  return (
    <div style={{
      background: "#fff", borderRadius: 14, border: "1.5px solid #F9731640",
      padding: "14px 20px", width: "100%", maxWidth: 520,
      boxShadow: "0 4px 24px #F9731615", display: "flex", alignItems: "center", gap: 16,
    }}>
      <motion.div
        animate={{ rotate: 360, x: [0, 6, -6, 4, -4, 0], y: [0, -6, 4, -4, 6, 0] }}
        transition={{
      ...
      <div style={{ fontFamily: "'Courier New', monospace", fontSize: 13, fontWeight: 700, color: "#F97316", flex: 1, minHeight: 20 }}>
        {typing && <TypewriterText key={currentIndex} text={`> ${line.cmd}  //  ${line.word}`} onDone={handleDone} />}
      </div>
    </div>
  );
}
...
export default function Home() {
  ...
  return (
    <main style={{ height: "100vh", background: "#FAF0E6", position: "relative", overflow: "hidden", display: "flex", flexDirection: "column" }}>
      <NeuralWaves />

      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 28px", position: "relative", zIndex: 10, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {["P", "Y", "A"].map((letter, i) => (
            <motion.div key={i}
              animate={{ y: [0, -5, 0] }}
              transition={{ duration: 2.2, repeat: Infinity, ease: "easeInOut", delay: i * 0.3 }}
              style={{ width: 52, height: 52, background: "#fff", border: "2px solid #F9731670", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 16px #F9731425" }}
            >
              <span style={{ fontSize: 24, fontWeight: 900, color: "#F97316", fontFamily: "'Courier New', monospace" }}>{letter}</span>
            </motion.div>
          ))}
          <motion.div animate={{ y: [0, -6, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}>
            <Image src="/pya002.png" alt="Pya" width={56} height={56} style={{ borderRadius: "50%", boxShadow: "0 4px 20px #F9731430" }} />
          </motion.div>
        </div>

        <motion.button onClick={handleAcesso}
          whileHover={{ scale: 1.05, boxShadow: "0 0 28px #F9731680" }}
          whileTap={{ scale: 0.97 }}
          style={{ background: "linear-gradient(135deg, #F97316, #ea580c)", color: "#fff", border: "none", borderRadius: 999, padding: "11px 28px", fontWeight: 800, fontSize: 14, cursor: "pointer", boxShadow: "0 4px 20px #F9731650", letterSpacing: 0.5 }}
        >
          Acessar
        </motion.button>
      </header>

      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14, padding: "0 24px", position: "relative", zIndex: 10 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
          style={{ display: "flex", alignItems: "center", background: "#fff", borderRadius: 999, border: "1.5px solid #F9731660", padding: "10px 16px", width: "100%", maxWidth: 460, boxShadow: "0 4px 20px #F9731615", gap: 10 }}>
          ...
          <motion.button onClick={handleAcesso}
            ...
            whileTap={{ scale: 0.95 }}
            style={{ background: "linear-gradient(135deg, #F97316, #ea580c)", color: "#fff", border: "none", borderRadius: 999, padding: "8px 20px", fontWeight: 800, fontSize: 13, cursor: "pointer", boxShadow: "0 2px 10px #F9731640" }}>
            Enviar
          </motion.button>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}
          ...
          {actionCards.map((card, i) => (
            <motion.button key={i} onClick={handleAcesso}
              whileHover={{ scale: 1.05, borderColor: "#F97316", color: "#F97316" }}
              whileTap={{ scale: 0.97 }}
              style={{ background: "#fff", border: "1.5px solid #e5d5c5", borderRadius: 999, padding: "8px 16px", fontSize: 13, fontWeight: 600, color: "#555", cursor: "pointer", transition: "all 0.2s", fontFamily: "'Georgia', serif" }}>
              {card}
            </motion.button>
          ))}
        </motion.div>
